...
func createSimString(instruction Instruction) string {
	...
	case "MOVK":
		...
		ProgramCounter += 4
	case "LSR":
		simOutput += fmt.Sprintf("LSR\tR%d, R%d, #%d\n", instruction.storeRegister, instruction.targetRegister1, instruction.immediateValue)
		registers[instruction.storeRegister] = int64(uint64(registers[instruction.targetRegister1]) >> uint64(instruction.immediateValue))

		ProgramCounter += 4
	case "LSL":
		simOutput += fmt.Sprintf("LSL\tR%d, R%d, #%d\n", instruction.storeRegister, instruction.targetRegister1, instruction.immediateValue)
		registers[instruction.storeRegister] = int64(uint64(registers[instruction.targetRegister1]) << uint64(instruction.immediateValue))
		ProgramCounter += 4
	case "STUR":
	...
	case "LDUR":
		...
		ProgramCounter += 4
	case "ASR":
		simOutput += fmt.Sprintf("ASR\tR%d, R%d, #%d\n", instruction.storeRegister, instruction.targetRegister1, instruction.immediateValue)
		registers[instruction.storeRegister] = int64(registers[instruction.targetRegister1]) >> uint64(instruction.immediateValue)
		ProgramCounter += 4
	case "NOP":
